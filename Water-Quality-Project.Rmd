...
Our main question for this assingment is: How accurately can we predict that a household will switch well
after creating different models using our variables.

Can we cause harm to individuals with this project?

This project could potentially cause harm under the following instances:

1. The data was not ethically sourced
2. Our project is used to make false claims despite our best intentions

#### The data

##### Are individuals identified in the data set?

No, the data is anonimized.

##### Are there issues of privacy or consent that we should address?

I do not belive so no. Anonimizing the data makes it difficult to trace who said what.

##### Are there sources of bias in the data?

This data is mostly fact (distance to a well) with some survey questions.

I would have to see the way that questions were worded exactly but as of now I have no reason to believe that
there are sources of bias in our data.
...
# distance is in meters!, that info is from paper
```
...
We are attempting to predict the switch variable
...
Each column is a variable while each row is an observation, thus the dataframe is tidy.
...
$\rightarrow$ What are the ranges of each of the numerical variables? Are the counts of households that switch wells and do not switch wells balanced or unbalanced? That is, do we have roughly equal numbers of households that switch wells and do not switch wells?
                                     ...
                                     mean    sd      min     max
arsenic                              1.66    1.11    0.51    9.65
distance                             48.3    38.5    0.387   340 
education                            4.83    4.02    0       17 
...
No, both distributions appear to be skew right.
...
Negative because there is a negative relationship between distance and households using that well.
...
This is the probability of switching at Distance being equal to 0:
This value is about 0.65.
...
Beta 1 is negative meaning that there is negative relationship between distance to a well and switching well.
Beta 1 / 4 = -0.1555, every 1 unit change in predictor, we expect a decrease in the probability of switching by about 16%.
...
As Arsenic increases in your current well, the more likely you are to switch

$\rightarrow$ Why did the coefficient for `distance` change when arsenic was added?

There is some relationship between arsenic and distance. This is why distance changed.
...
tidy(log_reg_fit)
```
association1 is not significant.

Potentially drop association from the model.
...
Better by about 4%. That isn't very impressive at all.
...
While our best model according to accuracy and sensitivity was XGBoost, I wonder how much better it would be if we remove the association variables as those variables seem to not contribute much to our model.
...
xgb_wf <- workflow() %>%
  add_formula(switch_well ~ distance + arsenic + education) %>%
  add_model(xgb_model)
...
xgb_fit %>%
  ...
  vip(geom = "col")
```
As you can see the all of the metrics went down. This allows me to conclude that while association variables are not significant they do in fact make the model slightly better.
...
After competing our analysis on the data, I conclude that while we can indeed create a model that predicts the odds of a household switching wells, it is just a little more accurate then the null model at roughly 60 to 65% despite trying different models. This is a very complex problem and I believe that in order to create better models we either need more samples of the same data or different types of variables that may be more indicative of a household switching wells. Certain factors I can think of off the top of my head are: Household size, a households transportation options, whether a household built their own well or not.
